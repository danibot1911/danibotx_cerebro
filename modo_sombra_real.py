...
def extraer_de_flashscore():
    try:
        ...
        partidos = []
        for div in soup.select(".event__match--live"):
            liga = div.get("data-league-name", "Liga desconocida")
            equipo_local = div.get("data-event-home", "Local")
            equipo_visitante = div.get("data-event-away", "Visitante")
            tiempo = div.select_one(".event__stage--block").text if div.select_one(".event__stage--block") else "Sin tiempo"
            partidos.append(f"[FLASH] {liga} - {equipo_local} vs {equipo_visitante} ({tiempo})")
